...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    i, num_dice, high = 1, 1, 0
    while i < 11:
        turn_score = averaged(i, dice)
        if turn_score > high:
            high = turn_score
            num_dice = i
        i += 1
...
def swap_strategy(score, opponent_score, margin=7, num_rolls=5):
...
def final_strategy(score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 12
    #Try to find out whether there is a beneficial swap
    benificial = 0
    for n in range(1,11):
    ...
    num_rolls = []
    for n in range(1,11):
        if EX(n, score, opponent_score) > free_bacon(opponent_score):
            num_rolls.append(n)
            roll_it = 1

    new_score = score + free_bacon(opponent_score)
    if free_bacon(opponent_score) + score >= 100 and is_swap(new_score, opponent_score):
        return 0
    elif benificial:
        num_rolls = choose_num_rolls
        return num_rolls
    elif roll_it and score <= 94:
        if len(num_rolls) >= 2:
            return num_rolls[1]
        else:
            return num_rolls[0]
